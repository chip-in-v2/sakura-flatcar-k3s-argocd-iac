...
使用方法:
  ...
  ./setup.py allow-ssh        パケットフィルタで ssh のアクセスを許可します
...
INFRA_APPS_CHART_VERSION = "0.1.8"
...
def _kubectl_apply_bytes_remote(
    ip: str,
    content: bytes,
    name: str = "manifest",
    retries: int = 5,
    ...
    """SSH 経由でリモートサーバに kubectl apply を bytes コンテンツで直接実行する。"""
    for attempt in range(retries + 1):
        result = subprocess.run(
            ["ssh", *SSH_OPTS, f"{FLATCAR_SSH_USER}@{ip}",
             "sudo kubectl apply -f -"],
...
def _kubectl_rollout_restart_remote(ip: str, resource: str, namespace: str, timeout: int = 300) -> None:
    """SSH 経由でリモートサーバの指定リソースを rollout restart し、Ready を待機する。"""
    subprocess.run(
        ["ssh", *SSH_OPTS, f"{FLATCAR_SSH_USER}@{ip}",
         f"sudo kubectl rollout restart {resource} -n {namespace}"
         f" && sudo kubectl rollout status {resource} -n {namespace} --timeout={timeout}s"],
        check=True,
    )
...
def _wait_for_argocd_app_synced_remote(ip: str, app_name: str, timeout: int = 300) -> None:
    """SSH 経由で ArgoCD Application の Sync が完了するまで待機する。

    Sync 状態と Health 状態を定期的に出力するため、OCI 認証失敗など
    問題が発生した際の診断に役立てられる。
    """
    script = r"""#!/bin/bash
set -e
...
echo "  ArgoCD Application '${APP}' の存在を確認中..."
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    sudo kubectl get application "$APP" -n argocd 2>/dev/null && break
    sleep 5
done
[ "$(date +%s)" -lt "$DEADLINE" ] || { echo "タイムアウト: Application '${APP}' が見つかりません" >&2; exit 1; }

echo "  ArgoCD Application '${APP}' の Sync を待機中..."
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    SYNC=$(sudo kubectl get application "$APP" -n argocd \
        -o jsonpath='{.status.sync.status}' 2>/dev/null || true)
    HEALTH=$(sudo kubectl get application "$APP" -n argocd \
        -o jsonpath='{.status.health.status}' 2>/dev/null || true)
    MSG=$(sudo kubectl get application "$APP" -n argocd \
        -o jsonpath='{.status.conditions[0].message}' 2>/dev/null || true)
    echo "  ${APP}: sync=${SYNC} health=${HEALTH}${MSG:+ msg=${MSG}}"
    [ "$SYNC" = "Synced" ] && exit 0
    sleep 10
done
echo "タイムアウト: Application '${APP}' の Sync が完了しませんでした" >&2
exit 1
"""
    subprocess.run(
        ["ssh", *SSH_OPTS, f"{FLATCAR_SSH_USER}@{ip}",
         f"bash -s -- {timeout} {app_name}"],
        input=script.encode(),
        check=True,
    )
    print(f"  {app_name}: Synced ✓")
...
def cmd_install_charts() -> None:
    ...
    print("==> SSH 接続が確立するまで待機中...")
    _wait_for_ssh(sv1_ip, FLATCAR_SSH_USER, timeout=300)
    ...
    print("==> infra-apps を OCI レジストリに push して ArgoCD に登録中...")
    _push_infra_apps_oci(sv1_ip, outputs)
    print("==> app-of-apps の Sync が完了するまで待機中...")
    _wait_for_argocd_app_synced_remote(sv1_ip, "app-of-apps")
...
# ---------------------------------------------------------------------------
# push-infra-apps ヘルパー
# ---------------------------------------------------------------------------


def _push_infra_apps_oci(sv1_ip: str, outputs: dict) -> None:
    """infra-apps を OCI レジストリに push し、app-of-apps を ArgoCD に登録する。

    install-charts と push-infra-apps の両方から呼び出される共通処理。
    """
    if not shutil.which("helm"):
        raise RuntimeError(
            "helm コマンドが見つかりません。\n"
            "インストール: curl https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3 | bash"
        )

    registry_fqdn: str = outputs["container_registry_fqdn"]["value"]
    push_user: str     = outputs["container_registry_push_user"]["value"]
    push_password: str = outputs["container_registry_push_password"]["value"]
    pull_user: str     = outputs["container_registry_pull_user"]["value"]
    pull_password: str = outputs["container_registry_pull_password"]["value"]

    infra_apps_yaml = os.path.join(RENDERED_DIR, "infra-apps.yaml")
    if not os.path.exists(infra_apps_yaml):
        raise RuntimeError(
            "rendered/infra-apps.yaml が見つかりません。"
            "先に install-charts を実行してください。"
        )

    tmpdir, tgz_path = _package_infra_apps_helm_chart(infra_apps_yaml)
    try:
        print(f"  パッケージ完了: {os.path.basename(tgz_path)}")
        subprocess.run(
            ["helm", "registry", "login", registry_fqdn,
        ...
        subprocess.run(
            ["helm", "push", tgz_path, f"oci://{registry_fqdn}"],
            check=True,
        )
        print(
            f"  push 完了: oci://{registry_fqdn}/"
            f"{INFRA_APPS_CHART_NAME}:{INFRA_APPS_CHART_VERSION}"
        )
    finally:
    ...
    repo_secret = (
        ...
        "kind: Secret\n"
        "metadata:\n"
        "  name: sakura-container-registry\n"
        "  namespace: argocd\n"
        "  labels:\n"
        ...
        "  type: helm\n"
        f'  url: "{registry_fqdn}"\n'
        f'  username: "{pull_user}"\n'
        f'  password: "{pull_password}"\n'
        '  enableOCI: "true"\n'
    )
    _kubectl_apply_bytes_remote(sv1_ip, repo_secret.encode(), "argocd-oci-repo-secret")
    ...
    app_of_apps = (
        ...
        "  project: infra\n"
        "  source:\n"
        f'    repoURL: "{registry_fqdn}"\n'
        f"    chart: {INFRA_APPS_CHART_NAME}\n"
        f'    targetRevision: "{INFRA_APPS_CHART_VERSION}"\n'
        "  destination:\n"
    ...
    _kubectl_apply_bytes_remote(sv1_ip, app_of_apps.encode(), "app-of-apps")
    print(
        f"  app-of-apps 登録完了: oci://{registry_fqdn}/"
        f"{INFRA_APPS_CHART_NAME}:{INFRA_APPS_CHART_VERSION}"
    )


# ---------------------------------------------------------------------------
# サブコマンド: push-infra-apps
# ---------------------------------------------------------------------------


def _package_infra_apps_helm_chart(infra_apps_yaml_path: str) -> tuple[str, str]:
    """rendered/infra-apps.yaml を Helm chart としてパッケージする。

    Returns:
        (tmpdir, tgz_path): 一時ディレクトリと .tgz ファイルパスのタプル。
        呼び出し元が tmpdir を shutil.rmtree で削除する責任を持つ。
    """
    tmpdir = tempfile.mkdtemp(prefix="infra-apps-chart-")
    chart_dir = os.path.join(tmpdir, INFRA_APPS_CHART_NAME)
    templates_dir = os.path.join(chart_dir, "templates")
    os.makedirs(templates_dir)

    chart_yaml = (
        "apiVersion: v2\n"
        f"name: {INFRA_APPS_CHART_NAME}\n"
        "description: ArgoCD Application リソース (infra apps)\n"
        "type: application\n"
        f"version: {INFRA_APPS_CHART_VERSION}\n"
    )
    with open(os.path.join(chart_dir, "Chart.yaml"), "w") as f:
        f.write(chart_yaml)

    # rendered infra-apps.yaml を templates/ にコピー
    # Helm テンプレート構文 ({{ }}) は含まれないためそのまま使用可
    shutil.copy2(infra_apps_yaml_path, os.path.join(templates_dir, "applications.yaml"))
    ...
    tgz_path = os.path.join(tmpdir, f"{INFRA_APPS_CHART_NAME}-{INFRA_APPS_CHART_VERSION}.tgz")
    if not os.path.exists(tgz_path):
        raise RuntimeError(
            f"Helm パッケージが見つかりません: {tgz_path}\n{result.stdout}\n{result.stderr}"
        )
    return tmpdir, tgz_path


def cmd_push_infra_apps() -> None:
    """infra-apps.yaml.tpl を変更した後に OCI レジストリを更新するコマンド。

    install-charts 完了後の差分更新専用。テンプレートを変更したら:
      1. terraform apply  (rendered/infra-apps.yaml を再生成)
      2. ./setup.py push-infra-apps
    """
    print("=" * 60)
    print("push-infra-apps: infra-apps を OCI レジストリに再 push します")
    print("=" * 60)

    print("==> Terraform outputs を取得中...")
    outputs = _get_terraform_output()
    node_public_ips: dict[str, str] = outputs["node_public_ips"]["value"]
    sv1_name = sorted(node_public_ips.keys())[0]
    sv1_ip   = node_public_ips[sv1_name]
    registry_fqdn: str = outputs["container_registry_fqdn"]["value"]
    print(f"  registry : {registry_fqdn}")
    print(f"  sv1      : {sv1_ip}")

    # argocd namespace の存在確認 (install-charts 未実行の早期検出)
    ns_check = subprocess.run(
        ["ssh", *SSH_OPTS, f"{FLATCAR_SSH_USER}@{sv1_ip}",
         "export PATH=$PATH:/opt/bin && kubectl get namespace argocd"],
        capture_output=True,
    )
    if ns_check.returncode != 0:
        if ns_check.stderr:
            print(ns_check.stderr.decode(errors="replace"), end="")
        if ns_check.stdout:
            print(ns_check.stdout.decode(errors="replace"), end="")
        raise RuntimeError(
            "argocd namespace が存在しません。\n"
            "先に './setup.py install-charts' を実行してから push-infra-apps を実行してください。"
        )

    print("==> テンプレートを再レンダリング中...")
    node_private_ips: dict[str, str] = outputs["node_private_ips"]["value"]
    lb_vip_ip: str = outputs["lb_global_ip"]["value"]
    chart_vars = _get_chart_vars()
    chart_vars["init_internal_ip"] = node_private_ips[sorted(node_private_ips.keys())[0]]
    chart_vars["lb_vip_ip"] = lb_vip_ip
    node_lb_ips_yaml = "\n".join(
        f'              - "{ip}"'
        for ip in sorted(node_public_ips.values())
    ...
    print("==> infra-apps.yaml を Helm chart としてパッケージ中...")
    _push_infra_apps_oci(sv1_ip, outputs)

    print()
    print("=" * 60)
    print("push-infra-apps 完了")
    print(
        f"ArgoCD が oci://{registry_fqdn}/{INFRA_APPS_CHART_NAME}:{INFRA_APPS_CHART_VERSION}"
    )
    print("から infra-apps を自動同期します。")
    print("=" * 60)
...
def cmd_allow_ssh() -> None:
    print("=" * 60)
    print("allow-ssh: SSH アクセスを許可します")
    print("=" * 60)
    ...
    for node_name in sorted(node_public_ips):
        ...
        if scan.returncode == 0 and scan.stdout.strip():
            _register_host_key(ip, scan.stdout)
            print(f"  {node_name} ({ip}): ホスト公開鍵を登録しました")
        else:
            print(f"  {node_name} ({ip}): ホスト公開鍵のスキャンに失敗しました (SSH 未起動の可能性)")
    _setup_ssh_config(node_public_ips)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  push-infra-apps    infra-apps を OCI レジストリに push して ArgoCD で同期します\n"
    ...
    subparsers.add_parser("push-infra-apps",  help="infra-apps を OCI レジストリに push して ArgoCD で同期します")
    subparsers.add_parser("allow-ssh",        help="パケットフィルタで SSH のアクセスを許可します")
    ...
    dispatch = {
        ...
        "push-infra-apps": cmd_push_infra_apps,
